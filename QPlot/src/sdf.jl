...
function compare_sdf(s; α_gatto=0.1, α_me=0.01, xmax=nothing)
    ...
    lines!(ax, xs, ys, label=L"J(\omega)= \frac{\pi}{2} \alpha \omega \frac{1}{\omega^2+1}\quad\alpha=%$(α_gatto)\quad\mathrm{[Gatto]}")
    # Me
    xs = collect(range([0,10]..., 1000))
    ys = [π/2 * α_me / scale * x^s * exp(-x) for x in xs]
    lines!(ax, xs, ys, label=L"J(\omega)= \frac{\pi}{2} \frac{\alpha}{\omega_s^s} \omega^s e^{-\omega}\quad\alpha=%$(α_me)\;s=%$(s)\quad\mathrm{[Me]}")

    vlines!(ax, trans_freq, label=L"\omega_s=%$(trans_freq)", color=:gray, linestyle=:dash)
    if !isnothing(xmax)
        xlims!(ax, 0, xmax)
    end
