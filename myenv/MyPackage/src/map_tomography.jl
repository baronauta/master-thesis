function tedopa_coefficients(sdf_filename)
    ...
    coefficients = T == 0 ? chainmapping_tedopa(sdf_filename) : chainmapping_ttedopa(sdf_filename)
    # save coefficients
...
function evolve_tomographic_states(sdf_filename, config_file, dir_name) 
    ...
    for case in tomoStates
        ...
        (sysenv,psi0) = defineSystem(sys_type="S=1/2", sys_istate=case, chain_size=config.chain_size, local_dim=config.local_dim);
        H = createMPO(sysenv, config.ϵ, config.Δ, "Sz", freqs_file, coups_file)
        # measurements
        # - Sx, Sy, Sz on reduced system
        operators = [LocalOperator(Dict(1 => "Sx")), LocalOperator(Dict(1 => "iSy")), LocalOperator(Dict(1 => "Sz"))]
...
function get_evolved_states(filename)
    ...
    rho_t = [ 0.5 * (σ0 + 2 * meas["Sx{1}_re"][i] * σ1 + 2 * meas["iSy{1}_im"][i] * σ2 + 2 * meas["Sz{1}_re"][i] * σ3 ) for i in 1:nmeas]
    return rho_t
end
...
function map_tomography(sdf_filename, ϵ, Δ; dt=0.01, tmax=30, growMPSval=10, local_dim=6)
    # Create directory to store data of map tomography, add config file
    dir_name, config_file = makedir_simul(sdf_filename, ϵ, Δ, dt, tmax, growMPSval, local_dim)
