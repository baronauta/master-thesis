function SpinBoson_evolution_TEBD(
    ...
    # Hs = ϵ⋅Z + Δ⋅X
    h_start =
        ϵ * op("Z", s[1]) * op("Id", s[2]) +
        Δ * op("X", s[1]) * op("Id", s[2]) +
    ...
    # - time<state>.dat for time step;
    # - meas<state>.dat for measurements of X, Y, Z on the TLS;
    # - norm<state>.dat for norm of the TLS;
    # - measN_<state>.dat for chain sites occupation;
    # - bondDims<state>.dat for monitoring the bond dimension
    # - measOcc_<state>.dat for occupation measurements

    if trackState
        ioMagMeas = open(joinpath(dir, "meas$state.dat"), "w")
        ioNormCheck = open(joinpath(dir, "norm$state.dat"), "w")
        ioTv = open(joinpath(dir, "time$state.dat"), "w")
    end

    if trackEnv
        ioPopMeas = open(joinpath(dir, "measN_$state.dat"), "w")
        ioOcc = open(joinpath(dir, "measOcc_$state.dat"), "w")
        ioTvEnv = open(joinpath(dir, "timeEnv_$state.dat"), "w")
    end

    if trackBondDim
        ioTrackBondDim = open(joinpath(dir, "bondDims$state.dat"), "w")
    end
    ...
    for (step, t) in enumerate(0.0:tau:ttotal)
        ...
        if (step - 1) % measStep == 0
            ...
            if trackState
                ...
                push!(spinMeasures, scalar(appo))
                # Write as X,Y,Z
                writedlm(ioMagMeas, transpose(spinMeasures), ',')
                flush(ioMagMeas)
            ...
            if trackEnv
                ...
                push!(occMeasures, scalar(appoOcc))
                writedlm(ioPopMeas, transpose(occMeasures), ',')
                flush(ioPopMeas)
                ...
                Threads.@threads for i = 1:numTSM
                    ...
                    if p2 <= max(20, lcone + 2)
                        ...
                        projLambdas = Lambdas
                        TSM[i] =
                            projectBetween(Lambdas, Gammas, projLambdas, projGammas, p2, p1)
                    end
                end
            ...
            if performProj
                ...
                Threads.@threads for i = 2:ChainLength+1
            ...
            if trackBondDim
                bondDim = [ITensors.dim(inds(a)[1]) for a in Lambdas]
...
function SingleEx_evolution_TEBD(
    Gammas,
    Lambdas,
    s;
    ϵ,
...
function MPS_evolution_TEBD(
    ...
    h_start =
        0.5 * freqs[1] * op("Z", s[1]) * op("Id", s[2]) +
    ...
    h_end =
        0.5 * freqs[ntot] * op("Z", s[ntot]) * op("Id", s[ntot-1]) +
