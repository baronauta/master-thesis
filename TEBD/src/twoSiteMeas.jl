...
function projectBetween(stateLambdas, stateGammas, projLambdas, projGammas, site1, site2)
    ...
    elseif pL == pR == size(stateGammas, 1)
        ...
        appo =
            stateLambdas[pL-1] * stateGammas[pR] * dag(projLambdas[pL-1] * projGammas[pR])
        return scalar(appo)
    elseif pL == pR && pL > 1 && pR < size(stateGammas, 1)
        ...
        if pL == 1
            ...
            prevState =
                stateLambdas[pL-1] *
                stateGammas[pL] *
                dag(projLambdas[pL-1] * prime(projGammas[pL], tounprimeR))
        end
        ...
        if pR < size(stateGammas, 1)
            for i = pL+1:pR
                ...
                prevState =
                    ...
                    stateLambdas[i-1] *
                    stateGammas[i] *
                    prime(dag(projLambdas[i-1] * projGammas[i]), [tounprimeL, tounprimeR])
            end
            ...
            appo =
                ...
                stateLambdas[pR-1] *
                stateGammas[pR] *
                dag(prime(projLambdas[pR-1], tounprimeR) * projGammas[pR])
            return scalar(appo)
        end
